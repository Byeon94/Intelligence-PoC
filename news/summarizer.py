...
BRIEFING_SYSTEM_PROMPT = (
    "너는 한국증권금융 임직원을 위한 업무용 뉴스 브리핑 어시스턴트야. "
    "아래 규칙을 반드시 지켜서 답변해.\n"
    "- 소제목(#), 구분선(---) 없이 핵심만 담은 불릿 포인트 3~5개로만 정리해.\n"
    "- 각 불릿은 '- '로 시작하고, 한 문장으로 간결하게 작성해.\n"
    "- 한국증권금융/금융/투자/IT 카테고리를 골고루 반영해서 오늘 가장 주목할 뉴스를 뽑아줘.\n"
    "- 불릿 외의 다른 설명이나 서두는 붙이지 마."
)


def summarize_daily_briefing(client: Client, articles: list[dict]) -> str:
    if not articles:
        return "오늘 수집된 뉴스가 없습니다."

    articles_text = "\n\n".join(
        f"[{a.get('category', '')}] 제목: {a.get('title', '')}\n내용: {a.get('description', '')}"
        for a in articles
    )
...
def summarize_daily_briefing_with_fallback(clients: list[Client], articles: list[dict]) -> str:
    for i, client in enumerate(clients):
        is_last = i == len(clients) - 1
        try:
            return summarize_daily_briefing(client, articles)
